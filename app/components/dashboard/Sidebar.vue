<script lang="ts" setup>
  ...
  const { chats = [], selectedChatId = "" } = defineProps<{
    chats?: ChatView[];
  ...
  const { userEmail, userProfilePicture, lastSeen, isOnline } =
...
<template>
  ...
  >
    <div
      :class="[
        'h-screen flex flex-col transition-all duration-300 ease-in-out overflow-hidden',
        selectedChatId ? 'px-2 py-4 md:py-6 md:px-3' : 'p-4 md:p-6',
      ]"
    >
      <div v-if="selectedChatId" class="h-full shadow-lg">
        <MinimizedSidebar
          :chats="chats"
      ...
      <div v-else class="flex flex-col h-full">
        <div class="shrink-0 mb-6">
          <h1
            class="text-2xl md:text-3xl font-bold text-text-primary mb-2 whitespace-nowrap"
          >
            Messages
          </h1>
          <p class="text-sm text-text-secondary whitespace-nowrap">
            Your conversations
          </p>
        </div>
